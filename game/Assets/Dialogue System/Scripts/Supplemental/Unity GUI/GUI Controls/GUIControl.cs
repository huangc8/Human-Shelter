...
namespace PixelCrushers.DialogueSystem.UnityGUI {
	...
	public class GUIControl : MonoBehaviour {
		...
		public virtual void DrawChildren(Vector2 relativeMousePosition) {
			...
			try {
				...
				if (isNavigationEnabled && (navigation.click != KeyCode.Space)) {
					if ((UnityEngine.Event.current.type == EventType.KeyDown) && (UnityEngine.Event.current.character == ' ')) return;
				}
		...
		private void UpdateLayoutChild(GUIControl child, Vector2 childWindowSize) {
			...
			child.UpdateLayout(); 
			if (!clipChildren) child.rect = new Rect(child.rect.x + rect.x, child.rect.y + rect.y, child.rect.width, child.rect.height);
		}
