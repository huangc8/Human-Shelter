...
namespace PixelCrushers.DialogueSystem.UnityGUI {
	...
	public class Navigation {
		...
		public bool IsClicked {
			get { return (UnityEngine.Event.current.type == EventType.KeyDown) && (UnityEngine.Event.current.keyCode == click); }
		}
		...
		private void CheckMouseWheel() {
			if (UnityEngine.Event.current.type == EventType.ScrollWheel) {
				mouseWheelY += UnityEngine.Event.current.delta.y;
			}
		}

		private bool IsNextControlInputDown(float axisValue) {
			if ((UnityEngine.Event.current.type == EventType.KeyDown) && (UnityEngine.Event.current.keyCode == next)) {
		...
		private bool IsPreviousControlInputDown(float axisValue) {
			if ((UnityEngine.Event.current.type == EventType.KeyDown) && (UnityEngine.Event.current.keyCode == previous)) {
				UnityEngine.Event.current.Use();
				isAxisPrevDown = true;
			} else if (mouseWheelY <= -mouseWheelSensitivity) {
				mouseWheelY = 0;
