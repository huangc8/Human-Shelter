...
namespace PixelCrushers.DialogueSystem.UnityGUI {
	...
	/// </summary>
	[AddComponentMenu("Dialogue System/UI/Unity GUI/Dialogue/Text Field UI")]
	public class UnityTextFieldUI : MonoBehaviour, ITextFieldUI {
		...
		void OnGUI() {
			if (control.visible) {
				...
				} else if (UnityEngine.Event.current.isKey) {
					if ((cancelKey != KeyCode.None) && (UnityEngine.Event.current.keyCode == cancelKey)) {
		...
		private bool IsAcceptKey() {
			if (IsKeyCodeReturn(acceptKey)) {
				return 
					UnityEngine.Event.current.Equals(UnityEngine.Event.KeyboardEvent("[enter]")) ||
						UnityEngine.Event.current.Equals(UnityEngine.Event.KeyboardEvent("return")) ||
						(UnityEngine.Event.current.isKey && (UnityEngine.Event.current.keyCode == KeyCode.KeypadEnter)) ||
						(UnityEngine.Event.current.isKey && (UnityEngine.Event.current.keyCode == KeyCode.Return)) ||
						((UnityEngine.Event.current.type == EventType.keyDown) && (UnityEngine.Event.current.character == '\n'));
			} else {
				return (acceptKey != KeyCode.None) && (UnityEngine.Event.current.keyCode == acceptKey);
			}
		}
