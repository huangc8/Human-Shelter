	...
	public class SequencerCommandCamera : SequencerCommand {
		...
		public void Update() {
			...
			if (DialogueTime.time < endTime) {
				...
				cameraTransform.rotation = Quaternion.Slerp(originalRotation, targetRotation, elapsed);
				cameraTransform.position = Vector3.Slerp(originalPosition, targetPosition, elapsed);
			} else {
				Stop();
			}
		}
