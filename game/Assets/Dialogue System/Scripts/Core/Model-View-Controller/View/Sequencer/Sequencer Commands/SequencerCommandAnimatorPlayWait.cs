...
namespace PixelCrushers.DialogueSystem.SequencerCommands {
	...
	public class SequencerCommandAnimatorPlayWait : SequencerCommand {
		...
		public void Start() {
			...
			if (animator == null) {
				if (DialogueDebug.LogWarnings) Debug.Log(string.Format("{0}: Sequencer: AnimatorPlayWait({1}, {2}, {3}): No Animator found on {2}", new object[] { DialogueDebug.Prefix, stateName, (subject != null) ? subject.name : GetParameter(1), crossfadeDuration }));
				Stop();
			} else {
