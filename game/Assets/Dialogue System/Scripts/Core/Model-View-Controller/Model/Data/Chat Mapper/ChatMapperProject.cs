...
namespace PixelCrushers.DialogueSystem.ChatMapper {
	...
	/// <summary>
	/// Defines an XML-serializable Chat Mapper project. See http://www.chat-mapper.com/
	...
	public class ChatMapperProject {
		...
		public static ChatMapperProject Load(TextAsset xmlFile) {
			#if UNITY_METRO
			Debug.LogWarning("ChatMapperProject.Load() is not supported in Windows Store apps.");
			return null;
			#else
		...
		/// <summary>
		/// Saves as XML with the specified filename.
		/// </summary>
		/// <param name="filename">Filename to save.</param>
		public void Save(string filename) {
			#if UNITY_METRO
			Debug.LogWarning("ChatMapperProject.Save() is not supported in Windows Store apps.");
			#else
			XmlSerializer xmlSerializer = new XmlSerializer(typeof(ChatMapperProject));
