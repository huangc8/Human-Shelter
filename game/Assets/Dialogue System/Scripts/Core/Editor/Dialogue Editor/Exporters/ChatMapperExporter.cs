...
namespace PixelCrushers.DialogueSystem {
	...
	public static class ChatMapperExporter {
		...
		public static void Export(DialogueDatabase database, string filename, bool exportActors, bool exportItems, bool exportLocations, bool exportVariables, bool exportConversations) {
			ChatMapperProject cmp = DatabaseToChatMapperProject(database, exportActors, exportItems, exportLocations, exportVariables, exportConversations);
		...
		private static List<ChatMapper.Conversation> ConversationsToCmp(DialogueDatabase database) {
			List<ChatMapper.Conversation> cmpConversations = new List<ChatMapper.Conversation>();
			database.conversations.ForEach(conversation => cmpConversations.Add(ConversationToCmp(conversation)));
		...
		private static List<ChatMapper.DialogEntry> DialogEntriesToCmp(List<DialogueSystem.DialogueEntry> entries) {
			List<ChatMapper.DialogEntry> cmpEntries = new List<DialogEntry>();
			entries.ForEach(entry => cmpEntries.Add(DialogEntryToCmp(entry)));
			return cmpEntries;
		}
